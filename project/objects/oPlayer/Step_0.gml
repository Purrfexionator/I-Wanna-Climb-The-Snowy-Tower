...
if (_water)
{
	...
	if (_water.object_index == oWater1 || _water.object_index == oWater3)
...
if (_platform)
{
	if _platform.can_drag velocity.x += _platform.hspeed;
	...
	if (vertical_direction == -1) _platTop += 2;
	...
	if (_abovePlatform && !place_meeting(x, _platStandingY, oBlock) && _platform.can_snap)
	{
		y = _platStandingY
		if (!_platform.can_drag && _platform.can_move) velocity.x += _platform.hspeed;
...
if (button_jump)
{
	...
	{
		//Trigger platform jump function
		if _platform with _platform on_jump();
...
if place_meeting(x + velocity.x, y + velocity.y, oBlock) {
	...
	if place_meeting(x, y + velocity.y, oBlock) {
		...
		if !place_meeting(x, y + sign(velocity.y) * 0.5, oBlock) y += sign(velocity.y) * 0.5;
		velocity.y = 0
	} else {
		y += velocity.y
	}
